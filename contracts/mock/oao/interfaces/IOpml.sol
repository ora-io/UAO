...
interface IOpml {
    ...
    function initOpmlRequest(bytes32 modelHash, bytes32 programHash, bytes calldata input)
        external
        returns (uint256 requestId);

    function uploadResult(uint256 requestId, bytes calldata output) external;

    function startChallenge(uint256 requestId, bytes calldata output, bytes32 finalState, uint256 stepCount)
        external
